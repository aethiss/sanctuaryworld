export const charactersDescriptions = {
  barbarian: {
    description: 'The Barbarian has unparalleled strength and expertly wields an entire arsenal in battle, with a weapon for every occasion. He bellows intimidating war cries and unleashes ground-shaking slams to send approaching hordes reeling.',
    videoMp4: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/barbarian-class-bg-loop-ultra.55857b6bc767bc60b8faeef24cb487c6.mp4',
    videoWeb: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/barbarian-class-bg-loop-ultra.c7b4d9b58f8d4ac62c269910f1996901.webm',
    youTubeId: '9DFgQ81vzGg',
  },
  sorcerer: {
    description: 'The Sorceress is described as a "high risk, high reward" style of hero, harnessing potent elemental magic, but being fragile. Their spells are designed around Fire, Cold, and Lightning abilities. Sorceress has a special mechanic of Crackling Energy.',
    videoMp4: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/sorcerer-class-bg-loop-ultra.91e110d785ff1eea9ff86437815bcde5.mp4',
    videoWeb: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/sorcerer-class-bg-loop-ultra.7606847fffab7724ff63d6b968195de3.webm',
    youTubeId: 'qPTOEGU04X4',
  },
  druid: {
    description: 'The Druid is a savage shapeshifter, fluidly transforming between the forms of a towering bear or a vicious werewolf to fight alongside the creatures of the wild. He also commands the power of earth, wind, and storm, unleashing nature’s wrath to devastating effect.',
    videoMp4: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/druid-class-bg-loop-ultra.281a2539b6e9d0a16463691225904d36.mp4',
    videoWeb: 'https://d3a5h34gwy5glx.cloudfront.net/assets/media/druid-class-bg-loop-ultra.bc0570bbae89a4a83cd0c265a3333efd.webm',
    youTubeId: 'uij9ysBKs4w',
  }
}
